...
for each in models_of_choice:
    print(
        f"\n Mean of model performance metrics accuracy is {each} is : " , (statistics.mean([x['accuracy_score'] for x in results[each]]))
        )


    print(
        f"Stadard Deviation of model performance metrics accuracy is {each} is : " , (statistics.pstdev([x['accuracy_score'] for x in results[each]])), "\n"
    )
